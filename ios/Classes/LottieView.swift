...
public class LottieView : NSObject, FlutterPlatformView {
   ...
   var delegates : [AnyValueDelegate]
   ...
   func setValue(type: String, value: String, keyPath: String) -> Void {
      ...
      case "ColorValue":
         ...
         self.animationView?.setValueDelegate(self.delegates[self.delegates.count - 1], for: AnimationKeypath(string: keyPath + ".Color"))
         break;
      case "OpacityValue":
         if let n = NumberFormatter().number(from: value) {
            let f = CGFloat(truncating: n)
            self.delegates.append(NumberDelegate(number: f))
            self.animationView?.setValueDelegate(self.delegates[self.delegates.count - 1], for: AnimationKeypath(string: keyPath + ".Opacity"))
         }
         break;
      default:
         break;
